  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-md bg-white rounded-2xl border border-slate-200
                   shadow-xl shadow-slate-200/80 overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div>
              <p className="text-sm font-semibold text-slate-900">{tanque.nome} — Arqueação</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className={`inline-block w-2 h-2 rounded-full ${cor.dot}`} />
                <span className="text-xs text-slate-400">{tanque.tipo}</span>
                {totalOverrides > 0 && (
          ...
          <div className="flex items-center gap-2">
            {salvando && <Loader2 className="w-3.5 h-3.5 text-slate-400 animate-spin" />}
            <button
              onClick={onFechar}
        ...
        <div className="grid grid-cols-[56px_1fr_1fr_32px] px-5 py-2 border-b border-slate-100 bg-slate-50">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">#</span>
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Altura (cm)</span>
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Volume (L)</span>
          <span />
        </div>
            ...
            return (
              <div
                ...
                className={`grid grid-cols-[56px_1fr_1fr_32px] items-center px-5 py-1.5 group
                            ...
                            ${isFeedback ? 'bg-emerald-50' : linha.isOverride ? 'bg-amber-50/60' : 'hover:bg-slate-50'}`}
              >
                {/* Índice */}
                ...
                >
                  {isEditandoAltura ? (
                    <input
                      ...
                      className="w-20 text-xs rounded border border-slate-300 bg-white
                                 px-1.5 py-1 text-slate-900 outline-none
                    ...
                    <span className={`text-xs font-mono select-none
                                     ${linha.isOverride ? 'text-amber-700 font-semibold' : 'text-slate-600'}
                ...
                >
                  {isEditandoVolume ? (
                    <input
                      ...
                      className="w-24 text-xs rounded border border-slate-300 bg-white
                                 px-1.5 py-1 text-slate-900 outline-none
                                 focus:border-slate-500 focus:ring-2 focus:ring-slate-100
                                 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none
                                 [&::-webkit-inner-spin-button]:appearance-none"
                    ...
                    {!isFeedback && linha.isOverride && !editando && (
                      <motion.button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-0.5 rounded
                                   text-slate-300 hover:text-amber-500 transition-all"
                      >
        ...
        <div className="px-5 py-3.5 border-t border-slate-100 bg-slate-50/50
          ...
          <button
            onClick={onFechar}
            className="text-xs px-3.5 py-1.5 rounded-lg bg-slate-900 text-white
