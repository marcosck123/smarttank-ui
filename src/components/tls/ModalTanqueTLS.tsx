...
import { X, Droplets, Thermometer, Gauge, Waves, ArrowDownToLine, Clock } from 'lucide-react'
...
export function ModalTanqueTLS({ leitura, onFechar }: Props) {
  const cor = COR_COMBUSTIVEL[leitura.tipo]

  const items = [
    { icon: <Gauge className="w-4 h-4" />,          label: 'Nível de produto', valor: `${leitura.alturaProdutoCm.toFixed(1)} cm` },
    { icon: <Waves className="w-4 h-4" />,          label: 'Nível de água',    valor: `${leitura.alturaAguaCm.toFixed(1)} cm`, alerta: leitura.alturaAguaCm >= 2.5 },
    { icon: <Droplets className="w-4 h-4" />,       label: 'Volume atual',     valor: formatarVolume(leitura.volumeLitros) },
      ...
      >
        ...
        <div className="p-6">
          <div className="flex gap-5">
            {/* Tanque visual */}
            <div className="relative w-24 h-56 rounded-2xl border-2 border-brown-200 bg-brown-50 overflow-hidden shrink-0">
              <motion.div
                className={`absolute bottom-0 w-full ${cor.barra} opacity-70`}
                initial={{ height: 0 }} animate={{ height: `${leitura.percentual}%` }}
                transition={{ duration: 0.4 }}
              />
              {/* camada de água */}
              {leitura.alturaAguaCm > 0 && (
                <div
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-lg font-bold text-brown-900 drop-shadow">{leitura.percentual.toFixed(0)}%</span>
              </div>
            </div>
              ...
              {items.map(it => (
                <div key={it.label} className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-sm font-semibold font-mono ${it.alerta ? 'text-red-600' : 'text-brown-800'}`}>
                      {it.valor}
                    </p>
                  </div>
                </div>
          ...
          <div className="mt-5 flex items-center gap-1.5 text-[11px] text-brown-400 justify-center">
            ...
            Atualizado {format(new Date(leitura.atualizadoEm), "HH:mm:ss", { locale: ptBR })} · TLS-450 Plus
          </div>
        </div>
      </motion.div>
    </motion.div>
